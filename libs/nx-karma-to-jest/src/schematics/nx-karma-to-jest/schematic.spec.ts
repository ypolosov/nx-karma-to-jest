...
describe('nx-karma-to-jest schematic', () => {
  let appTree: Tree;
  ...
  beforeEach(() => {
    ...
    appTree.overwrite(`package.json`, JSON.stringify(PACKAGE_JSON));
  ...
  it('should run successfully', async () => {
    await expect(
      runner.runSchematicAsync('nxKarmaToJest', {}, appTree).toPromise()
  ...
  describe('Create jest.config.js files', () => {
    ...
    it('on app level', async () => {
      ...
      expect(tree.exists('/apps/myapp/jest.config.js')).toBe(true);
    });
    ...
    it('on lib level', async () => {
      ...
      expect(tree.exists('/libs/lib2/jest.config.js')).toBe(true);
    });
    ...
    it('on nested lib level', async () => {
      ...
      expect(tree.exists('/libs/shared/lib1/jest.config.js')).toBe(true);
    });
  });
  ...
  describe('Create test-setup.ts files', () => {
    it('on app src level', async () => {
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();
      expect(tree.exists('/apps/myapp/src/test-setup.ts')).toBe(true);
    });
    ...
    it('on lib src level', async () => {
      ...
      expect(tree.exists('/libs/lib2/src/test-setup.ts')).toBe(true);
    });
    ...
    it('on nested lib src level', async () => {
      ...
      expect(tree.exists('/libs/shared/lib1/src/test-setup.ts')).toBe(true);
    });
  });

  describe('Deletes karma.conf.js files', () => {
    it('on root level', async () => {
      expect(appTree.exists('/karma.conf.js')).toBe(true);
    ...
    it('on app level', async () => {
      expect(appTree.exists('/apps/myapp/karma.conf.js')).toBe(true);
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();

      expect(tree.exists('/apps/myapp/karma.conf.js')).toBe(false);
    });

    it('on lib level', async () => {
      expect(appTree.exists('/libs/lib2/karma.conf.js')).toBe(true);
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();

      expect(tree.exists('/libs/lib2/karma.conf.js')).toBe(false);
    });

    it('on nested lib level', async () => {
      expect(appTree.exists('/libs/shared/lib1/karma.conf.js')).toBe(true);
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();

      expect(tree.exists('/libs/shared/lib1/karma.conf.js')).toBe(false);
    });
  });

  describe('Deletes test.ts files', () => {
    it('on app src level', async () => {
      expect(appTree.exists('/apps/myapp/src/test.ts')).toBe(true);
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();

      expect(tree.exists('/apps/myapp/src/test.ts')).toBe(false);
    });

    it('on lib src level', async () => {
      expect(appTree.exists('/libs/lib2/src/test.ts')).toBe(true);
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();

      expect(tree.exists('/libs/lib2/src/test.ts')).toBe(false);
    });

    it('on nested lib src level', async () => {
      expect(appTree.exists('/libs/shared/lib1/src/test.ts')).toBe(true);
      const tree = await runner
        .runSchematicAsync('nx-karma-to-jest', {}, appTree)
        .toPromise();

      expect(tree.exists('/libs/shared/lib1/src/test.ts')).toBe(false);
    });
  });
